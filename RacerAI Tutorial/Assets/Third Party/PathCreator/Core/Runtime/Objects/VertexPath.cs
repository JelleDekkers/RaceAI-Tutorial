...
namespace PathCreation {
    ...
    public class VertexPath {
        ...
        public readonly float[] progresses;
        ...
        VertexPath (BezierPath bezierPath, VertexPathUtility.PathSplitData pathSplitData, Transform transform) {
            ...
            for (int i = 0; i < localPoints.Length; i++) {
                ...
                progresses[i] = cumulativeLengthAtEachVertex[i] / length;
        ...
        public Vector3 GetPointAtDistance (float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop) {
            ...
            return GetPointAtProgress (progress, endOfPathInstruction);
        }
        ...
        public Vector3 GetNormalAtDistance (float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop) {
            float progress = dst / length;
            return GetNormal (progress, endOfPathInstruction);
        }
        ...
        public Quaternion GetRotationAtDistance (float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop) {
            float progress = dst / length;
            return GetRotation (progress, endOfPathInstruction);
        }

        /// Gets point on path based on 'progress' (where 0 is start, and 1 is end of path).
        public Vector3 GetPointAtProgress (float progress, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop) {
        ...
        /// Gets normal vector on path based on 'progress' (where 0 is start, and 1 is end of path).
        public Vector3 GetNormal (float progress, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop) {
            var data = CalculatePercentOnPathData (progress, endOfPathInstruction);
        ...
        public float GetClosestProgressOnPath (Vector3 worldPoint) {
            TimeOnPathData data = CalculateClosestPointOnPathData (worldPoint);
            return Mathf.Lerp (progresses[data.previousIndex], progresses[data.nextIndex], data.percentBetweenIndices);
        }
        ...
        TimeOnPathData CalculatePercentOnPathData (float t, EndOfPathInstruction endOfPathInstruction) {
            ...
            float abPercent = Mathf.InverseLerp (progresses[prevIndex], progresses[nextIndex], t);
